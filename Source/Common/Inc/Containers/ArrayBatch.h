...
template <typename First, typename... Rest> struct Get<0, First, Rest...> {
  Get() = delete;
  typedef First Type;
};

template <U32 Index, typename First, typename... Rest> struct Get<
    Index, First, Rest...> {
  Get() = delete;
...
template <U32 NumElements, typename... BatchProperties> class ArrayBatch {
  template <U32 Index> using Type = typename Impl::Get<
...
public:
  ...
  template <U32 Index> const Array<Type<Index>, NumElements> &Get() const;
...
template <U32 NumElements, typename ... BatchProperties> template <U32 Index
