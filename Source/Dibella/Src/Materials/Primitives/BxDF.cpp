...
namespace Dibella {
  BxDF::BxDF(Allocator* allocator, BxDFType type)
    : AllocationDependent(allocator),
      mType(type) {}

  Color3f BxDF::SampleF(const Vector3f& wo, Vector3f* wi, const Point2f& xi, Float* pdf,
  ...
  bool BxDF::IsType(BxDFType t) const { return (mType & t) == mType; }
};
